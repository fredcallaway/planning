...
def latin_hyper_square(*iv_lens):
...
def hyper_square(df, cols):
    labeled_cols = [c + '_' for c in cols]
    unique_lens = df[cols].apply(lambda x: len(x.unique())).as_matrix()
    design_matrix = latin_hyper_square(*unique_lens)
    for cond in design_matrix:
        dd = df.copy()
        idx = [tuple(x) for x in cond]
        dd[labeled_cols] = dd[cols].apply(lambda s: s.apply(Labeler()), axis=0)
        yield dd.set_index(labeled_cols).ix[idx].reset_index(drop=True)
